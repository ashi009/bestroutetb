...
function processRegionalDelegationFile(name, bufs, callback) {
  ...
  for (var match; match = kDelegationEntryPattern.exec(content); ) {
    ...
    var prefix = new Prefix(match[2], maskLength);
    delegationsByCountry[match[1]].push(prefix);
    delegationsByCountry[name].push(prefix);
  }
...
function generateDb(options, callback) {
  ...
  async.map(kRegionalDelegationFiles, function(file, callback) {
    async.waterfall([
      ...
      processRegionalDelegationFile.bind(null, file.name)
    ], callback);
